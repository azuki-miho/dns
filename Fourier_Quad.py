...
class Fourier_Quad:
    ...
    def shear_est(self, gal, psf, imagesize, background_noise=None, F=True):#False for x-space
    ...
    def set_bins(self,array,bin_num,model=1,sym=False):# The input must be one dimensional array.(1,n)
        ...
        else:  #set up bins but specifing the endpoint of each bins in the right side of the mean valve
            length = len(array)
            if length >20000:
                pick_out = 2000
            else:
                pick_out = length/10  #pick_out means to use distribution of the points that picked out  to replace the distribution of original data
            pick_out =numpy.random.randint(low=0,high=length,size=pick_out)
            mean = numpy.mean(array[pick_out]) 
